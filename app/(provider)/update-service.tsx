...
export default function UpdateService() {
  ...
  // Parse the service data passed through navigation
  const initialService = useMemo(
    () => (service ? JSON.parse(service as string) : null),
    [service]
  );
  ...
  // Update the useEffect hook in your component
  useEffect(() => {
    if (initialService) {
      // Convert duration from minutes to hours if it's a large number
      const duration = initialService.duration;
      let durationValue = duration?.toString() || "4"; // Default to "4" if undefined
      let durationUnit = "minutes";

      if (duration >= 60 && duration % 60 === 0) {
        durationValue = (duration / 60)?.toString();
        durationUnit = "hours";
      }
      ...
      setFormData((prev) => {
        const newFormData = {
          ...
          category_id: initialService.category_id?.toString(),
  ...
  }, [initialService]); // Make sure initialService is stable
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Call update service API
      await updateService({ data: payload, id: initialService.id }).unwrap();
  ...
  if (!initialService) {
    return (
      <SafeAreaView className="flex-1 bg-background items-center justify-center">
        <Text className="text-white">No service data provided</Text>
        <TouchableOpacity
  ...
  return (
    <SafeAreaView className="flex-1 bg-background pt-8">
      ...
      <View className="p-4 bg-background border-t border-[#3A3A50]">
        <TouchableOpacity
        ...
        >
          {isUpdating ? (
            ...
            <Text className="text-white text-lg font-bold">Update Service</Text>
          )}
        </TouchableOpacity>
      </View>
